(function() {
  ...
  module("Backbone.WAMPCollection", {

    setup: function() {
      a         = new Backbone.WAMPModel({id: 3, label: 'a'});
      b         = new Backbone.WAMPModel({id: 2, label: 'b'});
      c         = new Backbone.WAMPModel({id: 1, label: 'c'});
      d         = new Backbone.WAMPModel({id: 0, label: 'd'});
      e         = null;
      col       = new Backbone.WAMPCollection([a,b,c,d]);
      otherCol  = new Backbone.WAMPCollection();
    }
  ...
  test("String comparator.", 1, function() {
    var collection = new Backbone.WAMPCollection([
      {id: 3},
      {id: 1},
      {id: 2}
    ], {comparator: 'id'});
  ...
  test("new and parse", 3, function() {
    var Collection = Backbone.WAMPCollection.extend({
      parse : function(data) {
  ...
  test("clone preserves model and comparator", 3, function() {
    var Model = Backbone.WAMPModel.extend();
    var comparator = function(model){ return model.id; };

    var collection = new Backbone.WAMPCollection([{id: 1}], {
      model: Model,
  ...
  test("get with non-default ids", 5, function() {
    var MongoModel = Backbone.WAMPModel.extend({idAttribute: '_id'});
    var model = new MongoModel({_id: 100});
    var col = new Backbone.WAMPCollection([model], {model: MongoModel});
    equal(col.get(100), model);
    equal(col.get(model.cid), model);
    equal(col.get(model), model);
    equal(col.get(101), void 0);

    var col2 = new Backbone.WAMPCollection();
  ...
  test('get with "undefined" id', function() {
    var collection = new Backbone.WAMPCollection([{id: 1}, {id: 'undefined'}]);
    equal(collection.get(1).id, 1);
  });

  test("update index when id changes", 4, function() {
    var col = new Backbone.WAMPCollection();
    col.add([
      {id : 0, name : 'one'},
      {id : 1, name : 'two'}
    ]);
  ...
  test("add", 14, function() {
    ...
    e = new Backbone.WAMPModel({id: 10, label : 'e'});
    otherCol.add(e);
    ...
    var f = new Backbone.WAMPModel({id: 20, label : 'f'});
    var g = new Backbone.WAMPModel({id: 21, label : 'g'});
    var h = new Backbone.WAMPModel({id: 22, label : 'h'});
    var atCol = new Backbone.WAMPCollection([f, g, h]);
    ...
    var coll = new Backbone.WAMPCollection(new Array(2));
    var addCount = 0;
  ...
  test("add multiple models", 6, function() {
    var col = new Backbone.WAMPCollection([{at: 0}, {at: 1}, {at: 9}]);
  ...
  test("add; at should have preference over comparator", 1, function() {
    var Col = Backbone.WAMPCollection.extend({
    ...
    col.add(new Backbone.WAMPModel({id: 1}), {at:   1});

    equal(col.pluck('id').join(' '), '3 1 2');
  });

  test("can't add model to collection twice", function() {
    var col = new Backbone.WAMPCollection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
  ...
  test("add model to multiple collections", 10, function() {
    var counter = 0;
    var e = new Backbone.WAMPModel({id: 10, label : 'e'});
    ...
    var colE = new Backbone.WAMPCollection([]);
    colE.on('add', function(model, collection) {
      equal(e, model);
      equal(colE, collection);
    });
    var colF = new Backbone.WAMPCollection([]);
  ...
  test("add model with parse", 1, function() {
    ...
    var Col = Backbone.WAMPCollection.extend({model: Model});
    var col = new Col;
  ...
  test("add model to collection with sort()-style comparator", 3, function() {
    ...
    var tom = new Backbone.WAMPModel({name: 'Tom'});
    var rob = new Backbone.WAMPModel({name: 'Rob'});
    var tim = new Backbone.WAMPModel({name: 'Tim'});
    col.add(tom);
    col.add(rob);
    col.add(tim);
  ...
  test("add and remove return values", 13, function() {
    var Even = Backbone.WAMPModel.extend({
    ...
    equal(list.length, 2);
    ok(list[0] instanceof Backbone.WAMPModel);
  ...
  test("shift and pop", 2, function() {
    var col = new Backbone.WAMPCollection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
  ...
  test("events are unbound on remove", 3, function() {
    var counter = 0;
    var dj = new Backbone.WAMPModel();
    var emcees = new Backbone.WAMPCollection([dj]);
  ...
  test("remove in multiple collections", 7, function() {
    ...
    var passed = false;
    var e = new Backbone.WAMPModel(modelData);
    var f = new Backbone.WAMPModel(modelData);
    f.on('remove', function() {
      passed = true;
    });
    var colE = new Backbone.WAMPCollection([e]);
    var colF = new Backbone.WAMPCollection([f]);
    ok(e != f);
    ok(colE.length === 1);
    ok(colF.length === 1);
    colE.remove(e);
  ...
  test("remove same model in multiple collection", 16, function() {
    var counter = 0;
    var e = new Backbone.WAMPModel({id: 5, title: 'Othello'});
    ...
    var colF = new Backbone.WAMPCollection([e]);
  ...
  test("Colllection: non-persisted model destroy removes from all collections", 3, function() {
    var e = new Backbone.WAMPModel({title: 'Othello'});
  ...
  test("create with validate:true enforces validation", 3, function() {
    var ValidatingModel = Backbone.WAMPModel.extend({
      validate: function(attrs) {
        return "fail";
      }
    });
    var ValidatingCollection = Backbone.WAMPCollection.extend({
  ...
  test("create will pass extra options to success callback", 1, function () {
    var Model = Backbone.WAMPModel.extend({
      sync: function (method, model, options) {
        ...
        return Backbone.WAMPModel.prototype.sync.call(this, method, model, options);
      }
    });
  ...
  test("where and findWhere", 8, function() {
    var model = new Backbone.WAMPModel({a: 1});
    var coll = new Backbone.WAMPCollection([
      model,
      {a: 1},
      {a: 1, b: 2},
      {a: 2, b: 2},
  ...
  test ("reset with different values", function(){
    var col = new Backbone.WAMPCollection({id: 1});
    col.reset({id: 1, a: 1});
    equal(col.get(1).get('a'), 1);
  });

  test("same references in reset", function() {
    var model = new Backbone.WAMPModel({id: 1});
    var collection = new Backbone.WAMPCollection({id: 1});
  ...
  test("reset passes caller options", 3, function() {
    ...
    var col = new (Backbone.WAMPCollection.extend({ model: Model }))();
  ...
  test("add does not alter arguments", 2, function(){
    var attrs = {};
    var models = [attrs];
    new Backbone.WAMPCollection().add(models);
  ...
  test("Invalid models are discarded with validate:true.", 5, function() {
    ...
    collection.model = Backbone.WAMPModel.extend({
  ...
  test("multiple copies of the same model", 3, function() {
    var col = new Backbone.WAMPCollection();
    var model = new Backbone.WAMPModel();
    col.add([model, model]);
    equal(col.length, 1);
    col.add([{id: 1}, {id: 1}]);
    equal(col.length, 2);
  ...
  test("#964 - collection.get return inconsistent", 2, function() {
    var c = new Backbone.WAMPCollection();
  ...
  test("#1112 - passing options.model sets collection.model", 2, function() {
    var Model = Backbone.WAMPModel.extend({});
    var c = new Backbone.WAMPCollection([{id: 1}], {model: Model});
    ok(c.model === Model);
  ...
  test("null and undefined are invalid ids.", 2, function() {
    var model = new Backbone.WAMPModel({id: 1});
    var collection = new Backbone.WAMPCollection([model]);
    model.set({id: null});
  ...
  test("#1355 - `options` is passed to success callbacks", 2, function(){
    var m = new Backbone.WAMPModel({x:1});
    var col = new Backbone.WAMPCollection();
    var opts = {
      opts: true,
  ...
  test("#1655 - groupBy can be used with a string argument.", 3, function() {
    var collection = new Backbone.WAMPCollection([{x: 1}, {x: 2}]);
  ...
  test("#1655 - sortBy can be used with a string argument.", 1, function() {
    var collection = new Backbone.WAMPCollection([{x: 3}, {x: 1}, {x: 2}]);
  ...
  test("`sort` shouldn't always fire on `add`", 1, function() {
    var c = new Backbone.WAMPCollection([{id: 1}, {id: 2}, {id: 3}], {
      comparator: 'id'
    });
  ...
  test("set", function() {
    var m1 = new Backbone.WAMPModel();
    var m2 = new Backbone.WAMPModel({id: 2});
    var m3 = new Backbone.WAMPModel();
    var c = new Backbone.WAMPCollection([m1, m2]);
  ...
  test("set + merge with default values defined", function() {
    ...
    var col = new Backbone.WAMPCollection([m], {model: Model});
  ...
  test('merge without mutation', function () {
    ...
    var Collection = Backbone.WAMPCollection.extend({model: Model});
  ...
  test('`set` matches input order in the absence of a comparator', function () {
    var one = new Backbone.WAMPModel({id: 1});
    var two = new Backbone.WAMPModel({id: 2});
    var three = new Backbone.WAMPModel({id: 3});
    var collection = new Backbone.WAMPCollection([one, two, three]);
  ...
  test("`set` with non-normal id", function() {
    var Collection = Backbone.WAMPCollection.extend({
      model: Backbone.WAMPModel.extend({idAttribute: '_id'})
    });
  ...
  test("#1915 - `parse` data in the right order in `set`", function() {
    var collection = new (Backbone.WAMPCollection.extend({
      parse: function (data) {
  ...
  test("fetch will pass extra options to success callback", 1, function () {
    var SpecialSyncCollection = Backbone.WAMPCollection.extend({
      url: '/test',
      sync: function (method, collection, options) {
        _.extend(options, { specialSync: true });
        return Backbone.WAMPCollection.prototype.sync.call(this, method, collection, options);
      }
    });
  ...
  test("#2612 - nested `parse` works with `Collection#set`", function() {
    ...
    var Job = Backbone.WAMPModel.extend({
      constructor: function() {
        this.items = new Items();
        Backbone.WAMPModel.apply(this, arguments);
      },
    ...
    var Item = Backbone.WAMPModel.extend({
      constructor: function() {
        this.subItems = new Backbone.WAMPCollection();
        Backbone.WAMPModel.apply(this, arguments);
      },
      parse: function(attrs) {
    ...
    var Items = Backbone.WAMPCollection.extend({
      model: Item
    });
  ...
  test('_addReference binds all collection events & adds to the lookup hashes', 9, function() {
    ...
    var Collection = Backbone.WAMPCollection.extend({
      ...
      _addReference: function(model) {
        Backbone.WAMPCollection.prototype._addReference.apply(this, arguments);
        calls.add++;
      ...
      _removeReference: function(model) {
        Backbone.WAMPCollection.prototype._removeReference.apply(this, arguments);
        calls.remove++;
  ...
  test("modelId", function() {
    var Stooge = Backbone.WAMPModel.extend();
    var StoogeCollection = Backbone.WAMPCollection.extend({model: Stooge});
  ...
  test('Polymorphic models work with "simple" constructors', function () {
    var A = Backbone.WAMPModel.extend();
    var B = Backbone.WAMPModel.extend();
    var C = Backbone.WAMPCollection.extend({
  ...
  test('Polymorphic models work with "advanced" constructors', function () {
    var A = Backbone.WAMPModel.extend({idAttribute: '_id'});
    var B = Backbone.WAMPModel.extend({idAttribute: '_id'});
    var C = Backbone.WAMPCollection.extend({
      model: Backbone.WAMPModel.extend({
    ...
    C = Backbone.WAMPCollection.extend({
      model: function (attrs) {
        return attrs.type === 'a' ? new A(attrs) : new B(attrs);
      },
  ...
  test("#3039: adding at index fires with correct at", 3, function() {
    var col = new Backbone.WAMPCollection([{at: 0}, {at: 4}]);
  ...
  test("#3039: index is not sent when at is not specified", 2, function() {
    var col = new Backbone.WAMPCollection([{at: 0}]);
    col.on('add', function(model, col, options) {
  ...
  test("#set accepts options.at as a string", 1, function() {
    var collection = new Backbone.WAMPCollection([{id: 1}, {id: 2}]);
  ...
  test("removing models triggers `set` event once", 1, function() {
    var collection = new Backbone.WAMPCollection([{id: 1}, {id: 2}, {id: 3}]);
    collection.on('update', function() { ok(true); });
